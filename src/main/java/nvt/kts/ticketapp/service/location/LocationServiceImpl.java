...
public class LocationServiceImpl implements LocationService {
    ...
    @Override
    public PlainLocationDTO save(Long locationSchemeId) throws LocationSchemeDoesNotExist {
        LocationScheme locationScheme = locationSchemeRepository.findByIdAndDeletedFalse(locationSchemeId)
                .orElseThrow(() -> new LocationSchemeDoesNotExist(locationSchemeId));

        PlainLocationDTO savedLocation = ObjectMapperUtils.map
                (locationRepository.save(new Location(locationScheme)), PlainLocationDTO.class);
